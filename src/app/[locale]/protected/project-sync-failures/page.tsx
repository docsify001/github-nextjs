...
export default function ProjectSyncFailuresPage() {
  ...
  const fetchList = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/project-sync-jobs?status=failed&page=${page}&pageSize=${PAGE_SIZE}`,
        { credentials: "include" }
      );
      if (res.status === 401) {
  ...
  const handleRetry = async (jobId: string) => {
    setRetryingId(jobId);
    try {
      const res = await fetch(`/api/project-sync-jobs/${jobId}/retry`, {
        method: "POST",
        credentials: "include",
      });
      if (res.status === 401) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            <CardDescription>{t("projectDesc")}</CardDescription>
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading && !data ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t("colWebhook")}</TableHead>
                ...
                <TableBody>
                  {data.items.map((job) => (
                    <TableRow key={job.id}>
                      ...
                      <TableCell className="max-w-[200px] truncate text-muted-foreground text-sm" title={job.webhook_url ?? ""}>
                        {job.webhook_url ?? "—"}
                      </TableCell>
                      <TableCell>
