...
import { flattenSnapshots, MonthSnapshots, OneYearSnapshots } from "../projects";
...
export class SnapshotsService {
  ...
  async addSnapshot(
    ...
    if (existingSnapshot) {
      // 如果当日数据已存在，更新为最新值
      debug(
        `Updating existing snapshot for this day (${day}) with new data`
      );
      const updatedMonths = currentMonths.map((item) => {
        if (item.month === month) {
          return {
            ...item,
    ...
    function addMonth() {
      return [...currentMonths, { month, snapshots: [snapshotData] }];
    }
    function editMonth() {
      return currentMonths.map((item) => {
        if (item.month === month) {
          return { ...item, snapshots: [...item.snapshots, snapshotData] };
        }
        return item;
      });
    }
...
function unflattenKeyValueMap(map: Record<YearMonthDayKey, number>) {
  ...
  const months = Object.entries(byMonth).map(([month, snapshots]) => ({
    month: Number(month),
    snapshots: snapshots.map((snapshot) => pick(snapshot, ["day", "stars", "mentionableUsers", "watchers", "pullRequests", "releases", "forks"])),
  }));
  return months;
}
