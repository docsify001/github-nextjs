...
export interface RepoWebhookData {
  ...
  // 作者信息（与 openmcp authors 表对齐，avatar 为 OSS 地址，avatar_url 为 GitHub 头像地址）
  author?: {
    name?: string | null;
...
/**
 * 创建webhook数据对象
 * @param author 可选，来自 hall_of_fame，含 avatar(OSS)、avatar_url(GitHub) 等，供 openmcp 写入 authors 表
 */
...
/**
 * 创建webhook请求对象
 * @param author 可选，来自 hall_of_fame，含 avatar(OSS)、avatar_url(GitHub)，供 openmcp 写入 authors.avatar / authors.avatarUrl
 */
export function createRepoWebhookRequest(
  ...
  return {
    ...
    data: createRepoWebhookData(type, repo, processingStatus, meta, author),
  };
} 